...
-include("qrcode_erl.hrl").
-include("qrcode_erl_params.hrl").
...
encode(Bin, QUIET_ZONE) ->
	encode(Bin, 'M', QUIET_ZONE).
%
encode(Bin, ECC, QUIET_ZONE) when is_binary(Bin) ->
	...
	Matrix = qrcode_erl_matrix:embed_data(Params, Codewords),
	MaskedMatrices = qrcode_erl_mask:generate(Params, Matrix),
	Candidates = [qrcode_erl_matrix:overlay_static(Params, M) || M <- MaskedMatrices],
	{MaskType, SelectedMatrix} = qrcode_erl_mask:select(Candidates),
	...
	QRCode = qrcode_erl_matrix:finalize(Dim, FMT, VSN, QUIET_ZONE, SelectedMatrix),
	%% NOTE: Added "API" record
	#qrcode_erl{version = Version, ecc = ECC, dimension = Dim + QUIET_ZONE * 2, data = QRCode}.
...
choose_qr_params(Bin, ECLevel) ->
	...
	Dim = qrcode_erl_matrix:dimension(Version),
...
generate_ecc0(Bin, Count, TotalLength, BlockLength, Acc) when byte_size(Bin) >= BlockLength, Count > 0 ->
	...
	EC = qrcode_erl_reedsolomon:encode(Block, TotalLength - BlockLength),
...
alignment_patterns(Version) ->
	D = qrcode_erl_matrix:dimension(Version),
...
version_info_bits(#qr_params{version = Version}) when Version =< 40 ->
	BCH = qrcode_erl_reedsolomon:bch_code(Version, ?VERSION_INFO_POLY),
	<<Version:6, BCH:12>>.

format_info_bits(#qr_params{ec_level = ECLevel, mask = MaskType}) ->
	Info = (ecc(ECLevel) bsl 3) bor MaskType,
	BCH = qrcode_erl_reedsolomon:bch_code(Info, ?FORMAT_INFO_POLY),
